...
export async function fetchPatreonJson(url, options = {}) {
  ...
  // Require a JSON content type for API responses to avoid silently processing wrong payloads.
  if (!contentType.toLowerCase().includes("json")) {
    const preview = bodyText.slice(0, 120).replace(/\s+/g, " ").trim();
    const ct = contentType || "unknown";
    const suffix = preview ? `; body starts with: ${preview}` : "";
    throw new PatreonApiError(`Patreon returned non-JSON response (content-type: ${ct})${suffix}`, resp.status || 502);
  }
