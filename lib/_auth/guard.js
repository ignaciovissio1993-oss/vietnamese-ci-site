...
async function checkMembership(session, env) {
  ...
  const url = `${PATREON_IDENTITY_URL}?include=memberships.campaign,memberships.currently_entitled_tiers&fields%5Bmember%5D=patron_status`;
  ...
  try {
    ...
    for (const item of included) {
      ...
      const hasEntitledTiers = Array.isArray(entitledTiers) && entitledTiers.length > 0;
      if (
        campaignId === configuredCampaignId &&
        (status === "active_patron" || hasEntitledTiers)
      ) {
        return true;
      }
    }
